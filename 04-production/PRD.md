...
| 3 |Recommendation screen limited to reason, steps and one CTA | Must | Priority: Must have Acceptance criteria: Rrecommendation screen opens at /recommendation with a unique title and description in head(). It displays the recommendation headline, expected impact, confidence, reason, 2-3 steps, and a "Create follow-up" primary button. It does not show the overview chart, baseline stat grid, or experiment metrics. |
