...
| 3 |Recommendation screen limited to reason, steps and one CTA | Must | Priority: Must haveThe recommendation screen opens at /recommendation with a unique title and description in head().
It displays the recommendation headline, expected impact, confidence, reason, 2-3 steps, and a "Create follow-up" primary button.
It does not show the overview chart, baseline stat grid, or experiment metrics. |
| 4 | Follow-up creation and detail screen | Must | Priority: Must have Acceptance criteria: Creating a follow-up on the recommendation screen navigates to /follow-up and records the follow_up_created event.
The follow-up screen shows title, owner, due date, linked recommendation, status (Planned / In review), metric to move, target, and step checklist.
When no follow-up exists, /follow-up shows an empty state with a link to the recommendation. |
